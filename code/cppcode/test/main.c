...
//{
//    FILE *fp5;
//    if ((fp5 = fopen("D:\\course.txt", "r")) == NULL)
//    {
//        printf("文件无效\n");
//        exit(1);
//    }
...
void readLessonInformation() {
    const char *data =
            "数学学院 201407 数学建模            3    15   选修 张文博 40407 2014-2015学年第二学期4-16周 周二 08:30  10:20  2-533 80   40   经济学利器     数学物理方程、运筹学\n"
            "所有学院 201405 个人发展规划        4    15   必修 张大仙 40405 2014-2015学年第二学期4-16周 周五 18:30  20:20  2-212 80   28   个人目标管理   逃避自由\n"
            "电子工程 201402 信号与系统          4    15   必修 梁栋   40402 2014-2015学年第二学期4-16周 周三 18:30  20:20  2-126 80   23   通信基础       信号与系统第二版\n"
            "数学学院 201404 工程数学            4    15   必修 默会霞 40404 2014-2015学年第二学期4-16周 周四 18:30  20:20  2-330 80   13   理科基础       复变函数\n"
            "所有学院 201401 毛概                4    15   必修 齐英艳 40401 2014-2015学年第二学期4-16周 周二 18:30  20:20  2-235 80   16   思想政治       毛泽东传\n"
            "物理学院 201403 大学物理            4    15   必修 金光生 40403 2014-2015学年第二学期4-16周 周三 18:30  20:20  2-125 80   11   工科基础       大学物理教程\n"
            "所有学院 201406 体育专项            2    15   选修 野球帝 40406 2014-2015学年第二学期4-16周 周一 08:30  10:50  操场  80   10   强身健体       无课本";

    lessonNum = 0;

    // Using sscanf to read data from the string directly
    while (sscanf(data, "%s %s %s %d %d %s %s %s %s %s %s %s %s %d %d %s %s",
                  lesson[lessonNum].college, lesson[lessonNum].id, lesson[lessonNum].name,
                  &lesson[lessonNum].score, &lesson[lessonNum].times, lesson[lessonNum].nature,
                  lesson[lessonNum].teacherName, lesson[lessonNum].teacherID, lesson[lessonNum].term,
                  lesson[lessonNum].week, lesson[lessonNum].starT, lesson[lessonNum].stopT,
                  lesson[lessonNum].classroom, &lesson[lessonNum].stuNum, &lesson[lessonNum].renshu,
                  lesson[lessonNum].intro, lesson[lessonNum].messag) == 17) {
        lessonNum++;
        data = strchr(data, '\n');
        if (!data)
            break;  // No more lines
        data++;  // Move past the newline
    }
}
...
//{
//    FILE *fp5;
//    if ((fp5 = fopen("D:\\teach.txt", "r")) == NULL)
//    {
//        printf("文件无效\n");
//        exit(1);
//    }
...
void readTeacherInformation() {
    const char *data = "40401  人文学院  齐英艳  13053214189    1234         qichunyan@bupt.edu.cn\n"
                       "40402    信通院    梁栋  13353214198    1234         liangdong@bupt.edu.cn\n"
                       "40403    理学院  金光生  13424334344    1234     jinguangsheng@bupt.edu.cn\n"
                       "40404    理学院  默会霞  12342444555    1234                     afwergerf\n"
                       "40405    人文院  张大仙  13423141244    1234       zhangdaxian@bupt.edu.cn\n"
                       "40406    体育院  野球帝  23434444434    1234           yeqiudi@bupt.edu.cn\n"
                       "40407    理学院  张文博  12343458873    1234        zhangwenbo@bupt.edu.cn";

    teacherNum = 0;

    // Using sscanf to read data from the string directly
    while (sscanf(data, "%s %s %s %s %s %[^\n]",
                  teacher[teacherNum].id, teacher[teacherNum].college, teacher[teacherNum].name,
                  teacher[teacherNum].phone, teacher[teacherNum].password, teacher[teacherNum].email) == 6) {
        teacherNum++;
        data = strchr(data, '\n');
        if (!data)
            break;  // No more lines
...
//{
//    FILE *fp5;
//    if ((fp5 = fopen("D:\\student.txt", "r")) == NULL) {
//        printf("文件无效\n");
//        exit(1);
//    }
//    for (studentNum = 0; !feof(fp5); studentNum++)
//        fscanf(fp5, "%s%s%s%s%s%s%s%s%s%s\n", student[studentNum].id, student[studentNum].college,
//               student[studentNum].prof, student[studentNum].name, student[studentNum].phone,
//               student[studentNum].password, student[studentNum].email, student[studentNum].course1,
//               student[studentNum].course2, student[studentNum].course3);
//    fclose(fp5);
//}

void readStudentInformation()
{
    const char *data = "2014212901  人文学院    社会学    张一  11000000000  aaaaaa   zhang1@126.com           NULL     信号与系统           NULL\n"
                       "2014212902  人文学院    社会学    张二  10100000000  bbbbbb   zhang2@126.com   个人发展规划       数学建模           NULL\n"
                       "2014212930  管理学院  工商管理    刘四  30000000003  001010   zhang9@233.com           NULL           NULL           NULL";

    studentNum = 0;

    // Using sscanf to read data from the string directly
    while (sscanf(data, "%s %s %s %s %s %s %s %s %s %s",
                  student[studentNum].id, student[studentNum].college, student[studentNum].prof,
                  student[studentNum].name, student[studentNum].phone, student[studentNum].password,
                  student[studentNum].email, student[studentNum].course1, student[studentNum].course2,
                  student[studentNum].course3) == 10)
    {
        studentNum++;
        data = strchr(data, '\n');
        if (!data)
...
void openedAllCourses() // 查询所有已经开设的课程
{
    ...
    for (i = 0; i <= lessonNum; i++) {
        printf("%-9s%-7s%-20s%-5s%-5s%-5s%-7s%-6s%-28s%-5s%-7s%-7s%-6s%-5d%-5d%-15s%-26s\n", lesson[i].college,
               lesson[i].id, lesson[i].name, lesson[i].score, lesson[i].times, lesson[i].nature, lesson[i].teacherName,
               lesson[i].teacherID, lesson[i].term, lesson[i].week, lesson[i].starT, lesson[i].stopT,
               lesson[i].classroom, lesson[i].stuNum, lesson[i].renshu, lesson[i].intro, lesson[i].messag);
    }
...
void searchCourseNamet() // 根据课程名称查询 函数
{
    ...
    printf("%-9s%-7s%-20s%-5s%-5s%-5s%-7s%-6s%-28s%-5s%-7s%-7s%-6s%-5d%-5d%-15s%-26s\n", lesson[i].college,
           lesson[i].id, lesson[i].name, lesson[i].score, lesson[i].times, lesson[i].nature, lesson[i].teacherName,
           lesson[i].teacherID, lesson[i].term, lesson[i].week, lesson[i].starT, lesson[i].stopT, lesson[i].classroom,
           lesson[i].stuNum, lesson[i].renshu, lesson[i].intro, lesson[i].messag);
    printf("按任意键键回到教师功能选择界面\n");
    system("pause");
}
...
void searchStudentNamet() // 根据学生名称查询 函数
{
    ...
    printf("学生姓名  ID          所选课程        \n");
    printf("%-10s%-12s%-15s%-15s%-15s\n", student[i].name, student[i].id, student[i].course1, student[i].course2,
           student[i].course3);
    printf("按任意键键回到教师功能选择界面\n");
    system("pause");
}
...
void changeCourse() // 调用 修改课程信息 函数
{
    ...
    for (i = 0; i < lessonNum; i++) {
        if (strcmp(teacherID, lesson[i].teacherID) == 0)
            printf("%-9s%-7s%-20s%-5s%-5s%-5s%-7s%-6s%-28s%-5s%-7s%-7s%-6s%-5d%-5d%-15s%-26s\n", lesson[i].college,
                   lesson[i].id, lesson[i].name, lesson[i].score, lesson[i].times, lesson[i].nature,
                   lesson[i].teacherName, lesson[i].teacherID, lesson[i].term, lesson[i].week, lesson[i].starT,
                   lesson[i].stopT, lesson[i].classroom, lesson[i].stuNum, lesson[i].renshu, lesson[i].intro,
                   lesson[i].messag);
    }
    printf("\n请输入欲修改的课程编号：");
    scanf("%s", instruct);
    ...
    switch (determinant) {
                ...
                case 1:
                    ...
                    for (i = 0; i < lessonNum; i++)
                        fprintf(fs, "%-9s%-7s%-20s%-5s%-5s%-5s%-7s%-6s%-28s%-5s%-7s%-7s%-6s%-5d%-5d%-15s%-26s\n",
                                lesson[i].college, lesson[i].id, lesson[i].name, lesson[i].score, lesson[i].times,
                                lesson[i].nature, lesson[i].teacherName, lesson[i].teacherID, lesson[i].term,
                                lesson[i].week, lesson[i].starT, lesson[i].stopT, lesson[i].classroom, lesson[i].stuNum,
                                lesson[i].renshu, lesson[i].intro, lesson[i].messag);
                    fclose(fs);
                    break;
                case 2:
                    printf("\n请输入新的课程简介:");
        ...
        case 1:
            ...
            for (i = 0; i < lessonNum; i++)
                fprintf(fs, "%-9s%-7s%-20s%-5s%-5s%-5s%-7s%-6s%-28s%-5s%-7s%-7s%-6s%-5d%-5d%-15s%-26s\n",
                        lesson[i].college, lesson[i].id, lesson[i].name, lesson[i].score, lesson[i].times,
                        lesson[i].nature, lesson[i].teacherName, lesson[i].teacherID, lesson[i].term, lesson[i].week,
                        lesson[i].starT, lesson[i].stopT, lesson[i].classroom, lesson[i].stuNum, lesson[i].renshu,
                        lesson[i].intro, lesson[i].messag);
            fclose(fs);
            break;
    }
    printf("按任意键键回到教师功能选择界面\n");
...
void addCourse() // 教师添加课程
{
    ...
    fprintf(fp5, "%-9s%-7s%-20s%-5s%-5s%-5s%-7s%-6s%-28s%-5s%-7s%-7s%-6s%-5d%-5d%-15s%-26s\n", temp.college, temp.id,
            temp.name, temp.score, temp.times, temp.nature, teacher[teacherL].name, teacher[teacherL].id, temp.term,
            temp.week, temp.starT, temp.stopT, temp.classroom, temp.stuNum, temp.renshu, temp.intro, temp.messag);
    fclose(fp5);
    system("cls");
    printf("成功添加课程");
    system("pause");
...
void changeEmailT() // 更改Email函数
{
    ...
    for (i = 0; i < teacherNum; i++) {
        fprintf(fs, "%9s%10s%8s%13s%8s%30s\n", teacher[i].id, teacher[i].college, teacher[i].name, teacher[i].phone,
                teacher[i].password, teacher[i].email);
    }
    fclose(fs);
...
void searchStudentC() {
    printf("以下是您的选课结果\n");
    printf("%s  %s  %s  %s  %s \n", student[studentL].id, student[studentL].name, student[studentL].course1,
           student[studentL].course2, student[studentL].course3);
...
void chooseLessonFunction() {
        ...
        if (strcmp(id, lesson[i].id) == 0) {
            if (lesson[i].stuNum - lesson[i].renshu == 0) {
                ...
                if (strcmp(student[studentL].course1, "NULL") == 0 &&
                    strcmp(student[studentL].course2, lesson[i].name) != 0 &&
                    strcmp(student[studentL].course3, lesson[i].name) != 0 &&
                    (strcmp(lesson[L2].week, lesson[i].week) != 0 || lesson[L2].starT, lesson[i].starT != 0) &&
                    (strcmp(lesson[L3].week, lesson[i].week) != 0 || lesson[L3].starT, lesson[i].starT != 0)) {
                    ...
                    if (strcmp(student[studentL].course2, "NULL") == 0 &&
                        strcmp(student[studentL].course1, lesson[i].name) != 0 &&
                        strcmp(student[studentL].course3, lesson[i].name) != 0 &&
                        (strcmp(lesson[L1].week, lesson[i].week) != 0 || lesson[L1].starT, lesson[i].starT != 0) &&
                        (strcmp(lesson[L3].week, lesson[i].week) != 0 || lesson[L3].starT, lesson[i].starT != 0)) {
                        ...
                        if (strcmp(student[studentL].course3, "NULL") != 0 &&
                            strcmp(student[studentL].course2, "NULL") != 0 &&
                            strcmp(student[studentL].course1, "NULL") != 0) {
                            printf("您已经报满课程\n");
    ...
    for (i = 0; i < studentNum; i++)
        fprintf(fp4, "%12s%10s%10s%8s%13s%8s%17s%15s%15s%15s\n", student[i].id, student[i].college, student[i].prof,
                student[i].name, student[i].phone, student[i].password, student[i].email, student[i].course1,
...
void delCourse() // 学生删除课程 函数
{
    ...
    char instruct[81];
    printf("\n已选课程1      已选课程2    已选课程3 ");
    if (strcmp(student[studentL].course1, "NULL") == 0 && strcmp(student[studentL].course2, "NULL") == 0 &&
        strcmp(student[studentL].course2, "NULL") == 0) {
        printf("您没有已选的课程");
        printf("按任意键返回学生功能选择界面");
        system("pause");
