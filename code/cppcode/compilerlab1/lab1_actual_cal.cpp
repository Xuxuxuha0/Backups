...
//vector<string> ans; // 编译结果【改进：直接输出，不存】
...
////                constant_stack.push(postfix_vector[i]);
...
////                    constant_stack.pop();
////                    Node temp_node;
////                    temp_node.type = "constant"; //设为常量，因为这个值已经被算出来了
////                    temp_node.num = 1; //由于只是想要编译结果，这个值可算可不算
////                    constant_stack.push(temp_node);
//                }
//                    // 乘法操作符：编译、两次出栈、计算、入栈
//                else if (postfix_vector[i].op == '*') {
//                    cout << "pop ebx" << endl
//                         << "pop eax" << endl
//                         << "imul eax, ebx" << endl
//                         << "push eax" << endl;
...
////                    constant_stack.push(temp_node);
//                }
//                    // 赋值操作符：出栈两次（分别为计算结果、被赋值的数）
//                else {
////                    constant_stack.pop(); // 计算结果
//                    cout << "pop eax" << endl
//                         << "mov DWORD PTR [ebp-"
//                         << var_index[first_node.var] * 4
//                         << "], eax" << endl;
...
////    fstream file(argv[1], fstream::in); // 打开命令行参数指定的文件,只读模式
//    file.open("D:\\SelfDownLoad\\code\\cppcode\\compilerlab1\\test_lab1.txt", fstream::in); // 为什么相对路径不行
//    init_op_priority(); // 初始化操作符优先级表
//    while (!file.eof()) {
//        file >> word; // 读取一个单词(以空格、换行符等分割,不会读入空格等)
//        clear();
//        // int语句【假设只有 int a; 这种格式】
//        if (word == "int") {
//            int_return("int");
//            continue;
//        }
//            // return语句【假设只有 return a; 这种格式】
//        else if (word == "return") {
//            int_return("return");
//            continue;
//        }
